...
namespace Lab4.Model.Tasks.Individual
{
    ...
    class IndividualTasksB
    {
        ...
        public static string IndividualB4()
        {
            ...
            {
                { 0, "zero" }, { 1, "one" }, { 2, "two" }, { 3, "three"}, { 4, "four"}, { 5, "five" }, { 6, "six" }, { 7, "seven"}, { 8, "four"}, { 9, "nine"}, { 10, "ten"},
            };
            Random rand = new Random();
            int year = rand.Next(yearDict.Count - 1);
            int lower = 1, upper = 4, coefFactor = 10;
            int exceptionalYear = 1;
            string resYear = "";
            if (year == exceptionalYear)
            {
                resYear = yearDict[year] + " year";
            }
            else if (year % coefFactor <= lower && year % coefFactor >= upper)
            {
                resYear = yearDict[year] + " years old";
            }
            else
            {
                resYear = yearDict[year] + " years";
            }
            return resYear;
        }
